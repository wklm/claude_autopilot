...
class UsageLimitTimeParser:
    ...
    TIMEZONE_MAPPINGS: ClassVar[list[TimezoneMapping]] = [
    ...
    TIMEZONE_MAP: ClassVar[dict[str, str]] = {tz.abbreviation: tz.iana_name for tz in TIMEZONE_MAPPINGS}
    ...
    def parse_usage_limit_message(self, message: str) -> UsageLimitTimeInfo:
        ...
        best_result: ParsedTime | None = None
    ...
    def _parse_match(self, match: re.Match, pattern: str, pattern_idx: int, original_text: str) -> ParsedTime | None:
        ...
        if pattern_type == TimePatternType.SPECIAL_TIME:
            ...
            # Create datetime for today in the specified timezone
            target_time = datetime.now(tz).replace(hour=hour, minute=0, second=0, microsecond=0)
        ...
        if pattern_type == TimePatternType.RELATIVE_TIME:
        ...
        # Pattern: "until 15:00"
        if pattern_type == TimePatternType.ABSOLUTE_TIME:
            hour = int(match.group(1))
            minute = int(match.group(2))

            # Assume local timezone
            local_tz = datetime.now().astimezone().tzinfo
            target_time = datetime.now(local_tz).replace(hour=hour, minute=minute, second=0, microsecond=0)
        ...
        # ISO format
        if pattern_type == TimePatternType.ISO_FORMAT:
            try:
    ...
    def get_wait_duration(self, message: str) -> timedelta | None:
    ...
    def get_timezone_info(self, abbreviation: str) -> TimezoneMapping | None:
