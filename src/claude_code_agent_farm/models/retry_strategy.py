...
class RetryStrategy(BaseModel):
    ...
    last_retry_time: Optional[datetime] = Field(default=None)
    ...
    def calculate_next_delay(self) -> int:
        ...
        if self.current_attempt == 0:
            ...
            base_delay = min(
                self.initial_delay_seconds * (self.backoff_factor ** self.current_attempt),
...
class UsageLimitRetryInfo(BaseModel):
    ...
    parsed_retry_time: Optional[datetime] = Field(default=None)
    
    # Calculated retry time using strategy
    calculated_retry_time: Optional[datetime] = Field(default=None)
    ...
    def set_parsed_time(self, retry_time: Optional[datetime]) -> None:
