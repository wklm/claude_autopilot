...
class RestartHealthCheck(BaseModel):
    ...
    restart_timestamp: Optional[datetime] = Field(default=None)
    restart_duration_ms: Optional[int] = Field(default=None)
...
class RestartAttemptTracker(BaseModel):
    ...
    def can_restart(self) -> tuple[bool, Optional[str]]:
        ...
        if len(self.attempts) >= self.max_attempts:
            return False, f"Maximum restart attempts ({self.max_attempts}) reached in the last {self.attempt_window_seconds} seconds"
    ...
    def record_attempt(self, health_check: Optional[RestartHealthCheck] = None) -> None:
