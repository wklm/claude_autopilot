"""Event models for Claude Single Agent Monitor."""
...
from claude_code_agent_farm.models.base import EventModel
from claude_code_agent_farm.models.time import UsageLimitTimeInfo
...
class StatusChangeEvent(EventModel):
    ...
    event_type: EventType = Field(default=EventType.STATUS_CHANGED, const=True)
    previous_status: str = Field(..., description="Previous status")
    new_status: str = Field(..., description="New status")
    context_percentage: Optional[int] = Field(default=None, ge=0, le=100)
...
class CommandEvent(EventModel):
    ...
    target: Optional[str] = Field(default=None, description="Target (e.g., tmux session)")
...
class ErrorEvent(EventModel):
    ...
    event_type: EventType = Field(default=EventType.ERROR_OCCURRED, const=True)
