...
from claude_code_agent_farm.models.base import SerializableModel
...
class TimezoneMapping(BaseModel):
    ...
    utc_offset: Optional[str] = Field(default=None, description="UTC offset (e.g., -08:00)")
...
class ParsedTime(SerializableModel):
    ...
    meridiem: Optional[str] = Field(default=None, pattern=r"^(AM|PM|am|pm)$")
    timezone_str: Optional[str] = Field(default=None, description="Timezone string from input")
    timezone: Optional[str] = Field(default=None, description="Resolved IANA timezone")
    
    # Relative time components
    relative_amount: Optional[int] = Field(default=None, ge=0)
    relative_unit: Optional[str] = Field(default=None, pattern=r"^(hours?|minutes?|days?)$")
    
    # Result
    parsed_datetime: Optional[datetime] = Field(default=None, description="The parsed datetime")
...
class UsageLimitTimeInfo(SerializableModel):
    ...
    # Best result
    retry_time: Optional[datetime] = Field(default=None, description="When to retry")
