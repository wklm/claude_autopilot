...
from claude_code_agent_farm.models.health_check import (
    HealthStatus,
    HealthCheckType,
    HealthCheckResult,
    RestartHealthCheck,
...
from claude_code_agent_farm.models.watchdog import (
    WatchdogTimer,
    HungAgentDetector,
    WatchdogState,
    ActivityType,
...
class FlutterAgentMonitor:
    ...
    def __init__(self, settings: FlutterAgentSettings):
        ...
        self.restart_tracker = RestartAttemptTracker(
            max_attempts=5,
    ...
    def setup_tmux_session(self) -> None:
        ...
        # Create new session
        run(
            f"tmux new-session -d -s {self.settings.tmux_session} -n agent",
            check=True
        )
    ...
    def start_claude_agent(self) -> None:
        ...
        run(
            f"tmux send-keys -t {self.settings.tmux_session}:agent 'cd {self.settings.project_path}' Enter",
            check=True
        )
        ...
        run(
            f"tmux send-keys -t {self.settings.tmux_session}:agent '{claude_cmd}' Enter",
            check=True
        )
    ...
    def send_prompt(self) -> None:
        ...
        run(
            f"tmux send-keys -t {self.settings.tmux_session}:agent '{escaped_prompt}' Enter",
            check=True
        )
    ...
    def capture_pane_content(self) -> str:
        ...
        try:
            result = subprocess.run(
                [
                    "tmux", "capture-pane", "-t", 
                    f"{self.settings.tmux_session}:agent", "-p"
                ],
                capture_output=True,
                text=True,
                check=True
    ...
    def _handle_usage_limit(self, content: str) -> None:
        ...
        self.current_usage_limit_info = UsageLimitRetryInfo(
    ...
    def restart_agent(self) -> None:
        ...
        try:
            ...
            run(
                f"tmux send-keys -t {self.settings.tmux_session}:agent C-c",
                check=True
            )
            ...
            # Clear the pane
            run(
                f"tmux clear-history -t {self.settings.tmux_session}:agent",
                check=True
            )
    ...
    def get_status_display(self) -> Table:
        ...
        table = Table(
            title=f"Claude Single Agent Monitor - {datetime.now().strftime('%H:%M:%S')}",
            box=box.ROUNDED,
        )
        ...
        if self.session.is_waiting_for_limit and self.session.wait_until:
            ...
            if wait_remaining > 0:
                table.add_row(
                    "Waiting Until", 
                    self.time_parser.format_retry_time(self.session.wait_until)
                )
    ...
    def _create_checkpoint(self) -> None:
        ...
        try:
            ...
            checkpoint_data = {
                ...
                "last_usage_limit": self.session.last_usage_limit.model_dump() if self.session.last_usage_limit else None,
    ...
    def _perform_pre_restart_checks(self) -> list[HealthCheckResult]:
        ...
        try:
            result = subprocess.run(
                ["tmux", "has-session", "-t", self.settings.tmux_session],
                capture_output=True,
                check=False
            )
    ...
    def run(self) -> int:
        ...
        try:
            ...
            console.print(Panel(
                f"[bold cyan]Claude Single Agent Monitor[/bold cyan]\n\n"
                ...
                while self.running:
                    ...
                    if current_status != last_status:
                        ...
                        self.watchdog.feed(
                            ...
                            f"Status: {last_status.value} → {current_status.value}"
                        )
                        ...
                        elif current_status == AgentStatus.USAGE_LIMIT:
                            if not self.settings.wait_on_limit:
    ...
    def cleanup(self) -> None:
        ...
        console.print(Panel(
            ...
            title="Claude Single Agent Monitor",
            box=box.ROUNDED,
        ))
