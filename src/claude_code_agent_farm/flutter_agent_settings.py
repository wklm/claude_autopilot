...
class FlutterAgentSettings(BaseSettings):
    ...
    # Docker mode flag
    flutter_agent_docker: bool = Field(
        default=False,
        description="Running inside Docker container for Flutter development"
    )
    
    # Project configuration
    project_path: Path = Field(
        default=Path("/home/wojtek/dev/carenji"),
        description="Path to the carenji Flutter project directory"
    )
    ...
    wait_on_limit: bool = Field(
        default=True,
        description="Wait when usage limit is hit"
    )
    
    restart_on_complete: bool = Field(
        default=True,
        description="Restart when task completes"
    )
    
    restart_on_error: bool = Field(
        default=True,
        description="Restart when an error occurs"
    )
    ...
    idle_timeout: int = Field(
        ...
        ge=10,
        le=3600,
        description="Seconds before considering agent idle"
    )
    
    # tmux configuration
    tmux_session: str = Field(
        default="claude-carenji",
        description="tmux session name for carenji development",
    ...
    auto_resume_enabled: bool = Field(
        default=True,
        description="Use claude-auto-resume if available"
    )
    ...
    status_update_interval: int = Field(
        ...
        ge=1,
        le=10,
        description="Seconds between status display updates"
    )
    ...
    max_prompt_display_length: int = Field(
        ...
        ge=50,
        le=500,
        description="Maximum characters to display from prompt"
    )
    ...
    default_wait_hours: int = Field(
        ...
        ge=1,
        le=24,
        description="Hours to wait if usage limit time cannot be parsed"
    )
    ...
    firebase_project_id: str = Field(
        default="carenji-24ab8",
        description="Firebase project ID for carenji"
    )
    
    firebase_emulators_enabled: bool = Field(
        default=True,
        description="Enable Firebase emulators for local development"
    )
    
    firebase_emulator_host: str = Field(
        default="127.0.0.1",
        description="Firebase emulator host"
    )
    ...
    mcp_enabled: bool = Field(
        default=True,
        description="Enable Flutter MCP for AI assistance"
    )
    
    mcp_vmservice_port: int = Field(
        default=8182,
        description="Flutter VM service port for MCP"
    )
    
    mcp_dds_port: int = Field(
        default=8181,
        description="Flutter DDS port for MCP"
    )
    
    mcp_auto_detect: bool = Field(
        default=True,
        description="Auto-detect Flutter MCP availability"
    )
    ...
    carenji_test_coverage_threshold: int = Field(
        ...
        ge=0,
        le=100,
        description="Minimum test coverage percentage for carenji"
    )
    
    carenji_lint_on_save: bool = Field(
        default=True,
        description="Run Flutter analyzer on file save"
    )
    ...
    @classmethod
    def validate_prompt_file(cls, v: Optional[Path]) -> Optional[Path]:
    ...
    def validate_prompt_configuration(self) -> "SingleAgentSettings":
    ...
    def prompt_text(self) -> str:
        """Alias for prompt property to maintain compatibility."""
        return self.prompt
...
class DockerEnvironment(BaseSettings):
    ...
    HOME: Path = Field(default=Path("/home/claude"))
    USER: str = Field(default="claude")
    ...
    # Claude environment
    CLAUDE_DANGEROUSLY_SKIP_PERMISSIONS: str = Field(default="1")
    ENABLE_BACKGROUND_TASKS: str = Field(default="1")
    
    @property
    def is_docker(self) -> bool:
        """Check if running in Docker."""
        return self.HOSTNAME is not None and Path("/.dockerenv").exists()
    
    @property
    def container_info(self) -> str:
        """Get container information string."""
        if self.CONTAINER_NAME:
            return f"Container: {self.CONTAINER_NAME}"
        elif self.HOSTNAME:
            return f"Container ID: {self.HOSTNAME[:12]}"
