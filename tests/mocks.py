...
class MockFlutterAgentMonitor:
    ...
    def start_agent(self):
        ...
        self.session.events.append(SystemEvent(event_type=EventType.AGENT_STARTED, message="Mock agent started", details={}))

    def send_prompt(self):
        """Mock sending prompt."""
        self.session.status = AgentStatus.WORKING
        self.session.events.append(SystemEvent(event_type=EventType.PROMPT_SENT, message=self.settings.prompt_text, details={}))
    ...
    def restart_agent(self):
        ...
        self.session.runs += 1
        self.session.status = AgentStatus.READY
        self.session.events.append(SystemEvent(event_type=EventType.AGENT_RESTARTED, message=f"Restart #{self.session.runs}", details={}))
    ...
    def _parse_usage_limit(self, content: str) -> Optional[UsageLimitInfo]:
        ...
        return UsageLimitInfo(message=content, retry_time=retry_time)
...
class MockClaudeAPI:
    ...
    def _generate_response(self, message: str) -> str:
        ...
        if "error" in message_lower:
            return self.response_templates["error"].format(error_message="Mock error for testing")
...
class MockFirebaseEmulatorSuite:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "emulators": {name: emulator.get_status() for name, emulator in self.emulators.items()},
        }
    ...
    def where(self, field: str, op: str, value: Any) -> List[dict]:
        ...
        for doc_id, doc_data in self.data.items():
            if field in doc_data:
                if (
                    (op == "==" and doc_data[field] == value)
                    or (op == ">" and doc_data[field] > value)
                    or (op == "<" and doc_data[field] < value)
                ):
...
class MockAuthEmulator:
    ...
    def _create_test_users(self):
        ...
        test_users = [
            {"uid": "nurse_001", "email": "nurse@carenji.com", "role": "nurse", "displayName": "Nurse Johnson"},
            {"uid": "admin_001", "email": "admin@carenji.com", "role": "admin", "displayName": "Admin Smith"},
            {"uid": "family_001", "email": "family@carenji.com", "role": "family", "displayName": "John's Family"},
        ]
    ...
    def create_user(self, email: str, password: str, **kwargs) -> dict:
        ...
        user = {"uid": uid, "email": email, **kwargs}
...
class MockStorageEmulator:
    ...
    def list_files(self, prefix: str = "") -> List[dict]:
        ...
        return [info for path, info in self.files.items() if path.startswith(prefix)]
...
class MockFunctionsEmulator:
    ...
    def call(self, function_name: str, data: dict) -> dict:
        ...
        if function_name in self.functions:
            ...
            self.call_history.append(
                {"function": function_name, "data": data, "result": result, "timestamp": datetime.now().isoformat()}
            )
            return result
        else:
    ...
    def _vital_alerts(self, data: dict) -> dict:
        ...
        return {"success": True, "alerts": alerts, "notify_staff": len(alerts) > 0}

    def _family_notification(self, data: dict) -> dict:
        """Mock family notification."""
        return {"success": True, "notification_sent": True, "family_members_notified": data.get("family_ids", [])}
...
class MockScenarioRunner:
    ...
    def setup_carenji_development_scenario(self):
        ...
        # Configure responses
        self.claude.set_custom_response("medication reminder", self._generate_medication_feature_response())
