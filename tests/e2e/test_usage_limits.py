...
class TestUsageLimits:
    ...
    def test_multiple_usage_limits_in_session(self, monitor_with_usage_limit):
        ...
        # First usage limit
        with patch.object(monitor, "capture_pane_content", return_value="Usage limit. Try again at 3:00 PM PST"):
            monitor.check_agent_status()
        ...
        # Second usage limit
        with patch.object(monitor, "capture_pane_content", return_value="Usage limit. Try again at 5:00 PM PST"):
            monitor.check_agent_status()

        assert monitor.session.usage_limit_hits == 2

        # Check events recorded
        usage_events = [e for e in monitor.session.events if e.event_type == "usage_limit"]
    ...
    def test_usage_limit_recovery(self, monitor_with_usage_limit):
        ...
        with patch.object(monitor, "capture_pane_content", return_value="Working on the Carenji tests..."):
            status = monitor.check_agent_status()
    ...
    def test_real_usage_limit_scenario(self, docker_container):
        ...
        # Create tmux session
        subprocess.run(
            ["docker", "exec", docker_container, "tmux", "new-session", "-d", "-s", session_name], check=True
        )
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "exec", docker_container, "tmux", "capture-pane", "-t", session_name, "-p"],
                capture_output=True,
                text=True,
            )
            ...
            # Cleanup
            subprocess.run(
                ["docker", "exec", docker_container, "tmux", "kill-session", "-t", session_name], capture_output=True
            )
    ...
    def test_usage_limit_edge_cases(self, monitor_with_usage_limit):
        ...
        with patch.object(monitor, "capture_pane_content", return_value="Usage limit reached but no time given"):
            status = monitor.check_agent_status()
            # Should still detect as usage limit even without time
            assert status == AgentStatus.USAGE_LIMIT

        # Test with only time, no timezone
        with patch.object(monitor, "capture_pane_content", return_value="Usage limit. Try again at 3:45 PM"):
            monitor.check_agent_status()
            # Should use default timezone
            assert monitor.session.usage_limit_info is not None

        # Test midnight edge case
        with patch.object(monitor, "capture_pane_content", return_value="Usage limit. Try again at 12:00 AM PST"):
            monitor.check_agent_status()
            limit_info = monitor.session.usage_limit_info
            assert limit_info.retry_time.hour == 0
            assert limit_info.retry_time.minute == 0
