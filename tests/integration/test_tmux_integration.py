...
class TestTmuxIntegration:
    ...
    def tmux_session(self, tmux_session_name):
        ...
        # Create new session
        result = run(f"tmux new-session -d -s {tmux_session_name} -n test", check=False, capture_output=True)
    ...
    def test_create_tmux_session(self, tmux_session_name):
        ...
        # Create session
        result = run(f"tmux new-session -d -s {tmux_session_name} -n agent", capture_output=True)
    ...
    def test_send_keys_to_session(self, tmux_session):
        ...
        # Send echo command
        run(f"tmux send-keys -t {tmux_session}:test 'echo Hello Carenji' Enter", check=True)
    ...
    def test_capture_pane_content(self, tmux_session):
        ...
        commands = ["echo 'Testing Carenji Agent'", "echo 'Flutter development'", "echo 'Firebase backend'"]
    ...
    def test_tmux_configuration(self, tmux_session):
        ...
        # Check if set
        result = run(f"tmux show-options -t {tmux_session} -g mouse", capture_output=True)
    ...
    def test_multiple_windows(self, tmux_session):
        ...
        result1 = run(f"tmux capture-pane -t {tmux_session}:test -p", capture_output=True)
        assert "Agent window" in result1.stdout

        # Capture from second window
        result2 = run(f"tmux capture-pane -t {tmux_session}:monitoring -p", capture_output=True)
