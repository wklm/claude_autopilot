...
from claude_code_agent_farm.models_new.events import SystemEvent, EventType, AgentStatus
...
class TestAgentLifecycle:
    ...
    def test_send_prompt_to_agent(self, monitor):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            expected_call = [
                ...
                "send-keys",
                "-t",
                f"{monitor.settings.tmux_session_name}:0",
                monitor.settings.prompt_text,
                "Enter",
            ]
    ...
    def test_session_persistence(self, monitor, temp_dir):
        ...
        monitor.start_agent()
        monitor.session.runs = 2
        monitor.session.events.append(SystemEvent(event_type=EventType.AGENT_WORKING, message="Test event", details={"test_event": True}))
    ...
    def test_restart_maintains_context(self, monitor):
        ...
        # Record some context
        monitor.session.runs = 1
        monitor.session.events.append(SystemEvent(event_type=EventType.AGENT_WORKING, message="Previous run completed task X", details={"custom_event": True}))
        ...
        with patch.object(monitor, "_create_tmux_session"):
            with patch.object(monitor, "send_prompt") as mock_send:
                ...
                assert any(event.message == "Previous run completed task X" for event in monitor.session.events)
    ...
    def test_real_tmux_lifecycle(self, docker_container):
        ...
        result = subprocess.run(
            ["docker", "exec", docker_container, "tmux", "new-session", "-d", "-s", session_name], capture_output=True
        )
    ...
    def test_concurrent_monitoring(self, monitor):
        ...
        with patch.object(monitor, "capture_pane_content", side_effect=["Working...", ">> ", "Starting new task..."]):
            results = []
            for _ in range(3):
