...
""")
        ...
        firebase_config = {
            "emulators": {"auth": {"port": 9099}, "firestore": {"port": 8080}, "storage": {"port": 9199}}
        }
    ...
    def test_flutter_commands(self, docker_container, carenji_project_dir):
        ...
        subprocess.run(["docker", "cp", str(carenji_project_dir), f"{docker_container}:/tmp/"], check=True)

        project_path = f"/tmp/{carenji_project_dir.name}"

        # Test flutter --version
        result = subprocess.run(
            ["docker", "exec", docker_container, "flutter", "--version"], capture_output=True, text=True
        )
        ...
        # Test flutter doctor
        result = subprocess.run(
            ["docker", "exec", docker_container, "flutter", "doctor", "-v"], capture_output=True, text=True
        )
        assert result.returncode == 0

        # Test flutter analyze
        result = subprocess.run(
            ["docker", "exec", "-w", project_path, docker_container, "flutter", "analyze", "--no-fatal-infos"],
            capture_output=True,
            text=True,
            timeout=60,
        )
    ...
    def test_flutter_pub_commands(self, docker_container, carenji_project_dir):
        ...
        result = subprocess.run(
            ["docker", "exec", "-w", project_path, docker_container, "flutter", "pub", "get"],
            capture_output=True,
            text=True,
            timeout=120,
        )
        ...
        result = subprocess.run(
            ["docker", "exec", "-w", project_path, docker_container, "flutter", "pub", "deps"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0
    ...
    def test_dart_commands(self, docker_container):
        ...
        # Test dart --version
        result = subprocess.run(
            ["docker", "exec", docker_container, "dart", "--version"], capture_output=True, text=True
        )
        assert result.returncode == 0
...
"""
        result = subprocess.run(
            ["docker", "exec", docker_container, "sh", "-c", f"echo '{test_file}' | dart format"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0
    ...
    def test_firebase_cli(self, docker_container, carenji_project_dir):
        ...
        # Test firebase use
        result = subprocess.run(
            ["docker", "exec", "-w", project_path, docker_container, "firebase", "use"], capture_output=True, text=True
        )
    ...
    def test_git_commands(self, docker_container):
        ...
        # Test git --version
        result = subprocess.run(
            ["docker", "exec", docker_container, "git", "--version"], capture_output=True, text=True
        )
        assert result.returncode == 0
    ...
    def test_code_generation_tools(self, docker_container, carenji_project_dir):
        ...
        result = subprocess.run(
            ["docker", "exec", "-w", project_path, docker_container, "dart", "run", "build_runner", "--version"],
            capture_output=True,
            text=True,
            timeout=30,
        )
    ...
    def test_testing_tools(self, docker_container, carenji_project_dir):
        ...
        # Test flutter test
        result = subprocess.run(
            ["docker", "exec", "-w", project_path, docker_container, "flutter", "test", "--no-pub"],
            capture_output=True,
            text=True,
            timeout=60,
        )
...
"""
        ...
        # Run analyzer
        result = subprocess.run(
            ["docker", "exec", "-w", project_path, docker_container, "dart", "analyze"],
            capture_output=True,
            text=True,
            timeout=60,
        )
    ...
    def test_or_tools_availability(self, docker_container):
        ...
        result = subprocess.run(
            ["docker", "exec", docker_container, "python3", "--version"], capture_output=True, text=True
        )

        if result.returncode == 0:
            # Try to import ortools
            result = subprocess.run(
                ["docker", "exec", docker_container, "python3", "-c", "import ortools; print('OR-Tools available')"],
                capture_output=True,
                text=True,
            )
    ...
    def test_development_utilities(self, docker_container):
        ...
        for tool_name, command in utilities:
            result = subprocess.run(["docker", "exec", docker_container] + command, capture_output=True, text=True)
    ...
    def test_flutter_device_config(self, docker_container):
        ...
        result = subprocess.run(
            ["docker", "exec", docker_container, "flutter", "devices"], capture_output=True, text=True
        )
    ...
    def test_carenji_specific_setup(self, docker_container):
        ...
        result = subprocess.run(["docker", "exec", docker_container, "locale", "-a"], capture_output=True, text=True)
        ...
        result = subprocess.run(
            ["docker", "exec", docker_container, "ls", "/usr/share/zoneinfo/"], capture_output=True, text=True
        )
    ...
    def test_full_development_cycle(self, docker_container, carenji_project_dir):
        ...
        # 4. Format code
        result = subprocess.run(
            ["docker", "exec", "-w", project_path, docker_container, "dart", "format", "."],
            capture_output=True,
            timeout=30,
        )
