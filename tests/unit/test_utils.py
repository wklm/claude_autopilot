...
class TestUsageLimitTimeParser:
    ...
    def test_parse_time_with_timezone(self, parser):
        ...
        assert result.has_valid_time
        assert len(result.parse_results) > 0
        parsed = result.parse_results[0]
        assert parsed.hour == 15  # 3 PM in 24-hour format
    ...
    def test_parse_duration_based_limits(self, parser):
        ...
        result = parser.parse_usage_limit_message(message)
        assert result.has_valid_time
        assert result.wait_duration is not None
        # Should be approximately 2 hours
        assert abs(result.wait_hours - 2.0) < 0.02  # Within ~1 minute
        ...
        assert result.has_valid_time
        assert result.wait_duration is not None
        assert abs(result.wait_minutes - 30.0) < 1.0  # Within 1 minute

        # Can't parse complex durations with current implementation
        # Parser would need enhancement to handle "X hours and Y minutes"
    ...
    def test_timezone_mappings(self, parser):
        ...
        assert parser.TIMEZONE_MAP["PST"] == "America/Los_Angeles"
        assert parser.TIMEZONE_MAP["EST"] == "America/New_York"
        assert parser.TIMEZONE_MAP["UTC"] == "UTC"
        assert parser.TIMEZONE_MAP["GMT"] == "UTC"
    ...
    def test_parse_invalid_messages(self, parser):
        ...
        # Returns default UsageLimitTimeInfo with 1 hour wait
        assert result.has_valid_time
        assert result.wait_hours == 1.0  # Default wait time
...
class TestProjectChecks:
    ...
    def test_check_carenji_project(self, mock_carenji_project):
        ...
        other_flutter.mkdir()
        (other_flutter / "pubspec.yaml").write_text("name: other_app\ndependencies:\n  flutter:\n    sdk: flutter\n")
...
class TestFirebaseEmulators:
    ...
    def test_get_firebase_emulator_status(self):
        ...
        with patch("claude_code_agent_farm.utils.shell.run") as mock_run:
